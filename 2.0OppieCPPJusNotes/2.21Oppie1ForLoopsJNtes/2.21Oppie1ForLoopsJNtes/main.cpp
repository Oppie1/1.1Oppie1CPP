...
int main()
{
	cout << "Here we use a for loop that will print numbers (5-25). Then will go to next step in program." << endl;
	//It is important to note that often computer program will use blocks of code (like this for loop) to perform actions one
	//after another "stacking" the blocks. So in essence they are "nested" within main. Here is an example of just one block
	...
	//Create a for loop that lists numbers 0-25 that increments by 5 to do so. Display output to screen
	...
	//CODE:
		...
		//Display x to screen after it passes control statement and before it iterates.
		//CODE:
		

		//The first output would be 5. Then the loop will keep looping and printing out to screen like so 5,10,15,20,25. Then
		//The loop will have completed and go to next part of code (Here congratulations statement

	//Once loop has fully executed display a message saying thr for loop code block has fully executed and so this is 
	//next step of program outside of the for loop code block.

	//Create a message notify user that for loop has fully executed (code block) and that they are back in main.
	//CODE:
