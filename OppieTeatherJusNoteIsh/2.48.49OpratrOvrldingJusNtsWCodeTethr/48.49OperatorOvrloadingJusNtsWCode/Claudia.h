...
public:
	...
	//This function has a parameter of an integer. So in main the call must have an integer or the program wont run
...
//Declare a function that overloads the ^ operator of the Claudia class with a parameter of another Claudia class object (here b).
//The return of the function will be another/new Claudia OBJECT of the Claudia class (that's why we use Claudia at before operator function
//since the function is already in Claudia class.

//CODE:
	Claudia operator^(Claudia aao);
};

//red squiggly = actual error, must fix
//Green / yellow squiggly = warning, should investigate
//build succeeded = code compiles and runs correctly